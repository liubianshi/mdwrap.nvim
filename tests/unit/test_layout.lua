...
-- wrap_sentence=true 是「按宽填满」（gww 语义），settle 那一套回落／溢出一概不适用；
-- 且 cjk_break_at_punct_only 在此模式下强制失效——两者在中文里语义互斥，只在标点断就
-- 意味着行长由标点位置决定，填不满。故这里逐行填到 40 列，与下面的非严格组合逐字节相同。
check("wrap_sentence=true 按宽填满（settle 不适用）",
...
check("wrap_sentence=true 时 punct_only 失效（两种配置等价）",
  wrap("首先，经济合作与发展组织在过去十年间持续跟踪各成员实施的产业补贴规模结构变化趋势。",
    { width = 40, cjk_break_at_punct_only = false, wrap_sentence = true }),
  { "首先，经济合作与发展组织在过去十年间持续", "跟踪各成员实施的产业补贴规模结构变化趋势。" })
...
-- ============================================================
-- 短尾巴并入 + 回落下限的两侧（两条来自真实使用的回归）
-- ============================================================

-- 回落下限只拦「极短」落点：逗号在 48 列 / 100 列可用宽，回落后下一行 73 列正常，
-- 这种回落是纯赚，不该被误判成溢出。（阈值 1/2 会把它判错，只差 4 列。）
check("回落：48/100 的逗号保留，不溢出",
  wrap("美方按取得成本记录的头寸增量则是已经落地的投资，区内成员二零一九至二零二五年对美的这一头寸合计增加五千二百亿美元。",
    STRICT(100)),
  { "美方按取得成本记录的头寸增量则是已经落地的投资，",
    "区内成员二零一九至二零二五年对美的这一头寸合计增加五千二百亿美元。" })

-- 短尾巴并入：断在拟合区内最远的逗号上，本行自身无可指摘，却只给下一行留了 8 列的尾巴。
-- 那一截并进本行（超宽 ≤ avail/4），好过吐一个十几列的孤行。
check("短尾巴并入本行（允许有限超宽）",
  wrap("投出的一侧同样如此，中国的对外直接投资头寸至今仍有六成记在中国香港名下，十年未降。这一偏离不是随机的记账噪声，各自记录到的投向对方的投资持续增加。",
    STRICT(80)),
  { "投出的一侧同样如此，中国的对外直接投资头寸至今仍有六成记在中国香港名下，十年未降。",
    "这一偏离不是随机的记账噪声，各自记录到的投向对方的投资持续增加。" })

-- 尾巴不短时不并入：句号远在 avail/4 之外，维持原落点，不把整段黏成一行。
check("尾巴不短则不并入",
  wrap("甲乙丙丁戊己庚辛壬癸子丑寅卯，辰巳午未申酉戌亥甲乙丙丁戊己庚辛壬癸子丑寅卯辰巳午未。",
    STRICT(40)),
  { "甲乙丙丁戊己庚辛壬癸子丑寅卯，",
    "辰巳午未申酉戌亥甲乙丙丁戊己庚辛壬癸子丑寅卯辰巳午未。" })

-- 末行短是正常的：尾巴一路到段尾都没有句级标点，不并入（否则每段最后一行都会被黏走）。
check("末行短不触发并入",
  wrap("甲乙丙丁戊己庚辛壬癸子丑寅卯，辰巳午未", STRICT(40)),
  { "甲乙丙丁戊己庚辛壬癸子丑寅卯，", "辰巳午未" })

-- 本行已落句末则无短尾巴问题：尾巴「酉戌。」只有 6 列，够得上并入的宽度门槛，但本行行尾
-- 已经是句号，并入只会把两句黏成一行——守卫拦住它，「一句一行」不受影响。
check("已落句末不并入（一句一行不受影响）",
  wrap("甲乙丙丁戊己庚辛壬癸子丑寅卯辰巳午未申。酉戌。", STRICT(40)),
  { "甲乙丙丁戊己庚辛壬癸子丑寅卯辰巳午未申。", "酉戌。" })
-- 对照：拟合区内有多个句级标点时取**最远**的，一行可含多个短分句（DECISIONS 第 9 节），
-- 这与上面那条并不矛盾——它是 sent 槽的既有行为，不经短尾巴并入。
check("拟合区内多个句号取最远（既有行为）",
  wrap("首先。其次。经济合作与发展组织在过去十年间持续跟踪各成员实施的产业补贴规模。", STRICT(40)),
  { "首先。其次。", "经济合作与发展组织在过去十年间持续跟踪各成员实施的产业补贴规模。" })

-- 禁后断标点后面紧跟超宽 token：PUSH 退无可退时不得让「《」独占一行。
-- 判据是「退回去的落点可不可断」，不是「退没退回到 i」。
check("PUSH 退无可退不破禁则", layout.wrap({
  { text = "。", width = 2, class = "punct_no_break_before" },
  { text = "《", width = 2, class = "punct_no_break_after" },
  { text = "https://example.com/" .. string.rep("p", 40), width = 60, class = "word" },
  { text = "》", width = 2, class = "punct_no_break_before" },
}, STRICT(20)),
  { "。", "《https://example.com/pppppppppppppppppppppppppppppppppppppppp》" })
